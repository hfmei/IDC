...
from model.utils import extract, default, extract_t_1
...
class BrownianBridgeModel(nn.Module):
    def __init__(self, model_config):
        ...
        self.cls_loss = model_config.cls_loss
        ...
        self.to_normal = model_config.to_normal
    ...
    def forward(self, x, y, dataset, labels, context=None):
        if self.to_normal:
            x = (x - 0.5) * 2.
            x.clamp_(-1., 1.)
            y = (y - 0.5) * 2.
        ...
        return self.p_losses(x, y, context, t, dataset, labels)

    def p_losses(self, x0, y, context, t, dataset, labels, noise=None):
        """
        model loss
        ...
        prototypes = dataset.prototypes.to(x0.device)
        prototypes = (prototypes - 0.5) * 2.
        prototypes.clamp_(-1., 1.)

        objective_list = []
        for prototype in prototypes:
            objective_p = self.q_singlestep(x0, prototype, y, t, noise)
            objective_list.append(objective_p)
        objective_all = torch.stack(objective_list).to(x0.device).permute(1, 0, 2, 3, 4)

        objective_distence_all = torch.mean((objective_all -
                                      objective_recon.unsqueeze(1).repeat(1, objective_all.size(1), 1, 1,
                                                                          1)).abs(), dim=(2, 3, 4))

        objective_distence = torch.mean((objective - objective_recon).abs(), dim=(-3, -2, -1))

        one_hot_labels = F.one_hot(labels, num_classes=self.num_classes).to(x0.device)
        masked_distances = objective_distence_all.where(one_hot_labels == 0, torch.tensor(float('inf'), device=x0.device))
        min_values_except_label = masked_distances.min(dim=1).values
        min_values, _ = objective_distence_all.min(dim=1)
        is_label_min = objective_distence == min_values
        objective_mask = torch.where(is_label_min, torch.tensor(0.0, device=x0.device), min_values_except_label)
        alpha = -0.2
        loss_sim = alpha * objective_mask.mean()

        if self.loss_type == 'l1':
            if self.cls_loss:
                loss = (objective - objective_recon).abs().mean() + loss_sim
            else:
        ...
        return loss, recloss, simloss, log_dict
    ...
    def q_singlestep(self, x0, prototype, y, t, noise):
        m_t = extract(self.m_t, t, x0.shape)
        m_t_1 = extract_t_1(self.m_t, t - 1, x0.shape)
        var_t = extract(self.variance_t, t, x0.shape)
        sigma_t = torch.sqrt(var_t)
        objective = m_t_1 * (prototype - x0) + m_t * (y - prototype) + sigma_t * noise
        return objective
