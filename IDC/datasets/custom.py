...
class CustomCIFARDataset(Dataset):
    def __init__(self, data_config, stage='train'):
        ...
        # if stage == 'train':
        #     partition_idx = np.random.RandomState(0).choice(len(self.dataset), 640, replace=False)
        #     self.dataset = Subset(self.dataset, partition_idx)

        dims = 3 * data_config.dataset_config.image_size * data_config.dataset_config.image_size
        num_class = self.num_classes
        torch.manual_seed(0)
        initial_tensor = torch.randn(num_class, dims)
        q, _ = torch.qr(initial_tensor.T)
        orthogonal_proto = [q[:, i].view(3, data_config.dataset_config.image_size,
                                         data_config.dataset_config.image_size) for i in range(num_class)]
