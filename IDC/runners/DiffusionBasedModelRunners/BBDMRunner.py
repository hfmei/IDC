...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def initialize_optimizer_scheduler(self, net, config):
        ...
        if config.model.auto_lr_scheduler:
    ...
    def loss_fn(self, net, batch, epoch, step, dataset, opt_idx=0, stage='train', write=True):
        ...
        loss, recloss, simloss, additional_info = net(x, x_cond, dataset, label)
        if write:
    ...
    def sample_to_eval(self, net, test_loader, sample_path, prototypes, adversary):
        ...
        acc_sample_dist_adv = torch.tensor(0.0, device=self.config.training.device[0])
        acc_cond_clean = torch.tensor(0.0, device=self.config.training.device[0])

        for batch in pbar:
            adv_batch = adversary(batch, self.config.model.attack)
            # adv_batch = adversary(batch, "autoattack")
            (x_cond, x, label) = adv_batch
            (x_cond_clean, x_clean, label_clean) = batch  #clean
            x_cond_clean = x_cond_clean.to(self.config.training.device[0])  #clean
            if self.config.training.use_DDP:
                sample = self.net.module.sample(x_cond, clip_denoised=self.config.testing.clip_denoised).to('cpu')
                sample_clean = self.net.module.sample(x_cond_clean, clip_denoised=self.config.testing.clip_denoised).to('cpu')
            else:
                sample = self.net.sample(x_cond, clip_denoised=self.config.testing.clip_denoised).to('cpu')
                sample_clean = self.net.sample(x_cond_clean, clip_denoised=self.config.testing.clip_denoised).to('cpu')

            if self.config.data.dataset_config.to_normal:
                sample = sample.mul_(0.5).add_(0.5).clamp_(0, 1.)
                sample_clean = sample_clean.mul_(0.5).add_(0.5).clamp_(0, 1.)

            for i in range(len(sample_clean)):
                x_sample = sample[i: i + 1]
                x_sample_clean = sample_clean[i: i + 1]

                y_sample_dist = torch.argmin(torch.mean((x_sample.repeat(
                    prototypes.size()[0], 1, 1, 1) - prototypes).abs(), dim=(1, 2, 3)))
                y_sample_clean = torch.argmin(torch.mean((x_sample_clean.repeat(
                    prototypes.size()[0], 1, 1, 1) - prototypes).abs(), dim=(1, 2, 3)))

                if y_sample_dist.item() == label[i] and y_sample_clean.item() == label_clean[i]:
                    acc_sample_dist_adv = acc_sample_dist_adv + 1.0
                if y_sample_clean.item() == label_clean[i]:
                    acc_cond_clean = acc_cond_clean + 1.0
        if self.config.training.use_DDP:
            dist.all_reduce(acc_cond_clean, op=dist.ReduceOp.SUM)
            dist.all_reduce(acc_sample_dist_adv, op=dist.ReduceOp.SUM)

        num_total = test_loader.dataset.dataset.data.shape[0]
        acc_sample_dist_adv = acc_sample_dist_adv / num_total
        acc_cond_clean = acc_cond_clean / num_total
        print("The final acc_clean:{:.3f}, acc_adv:{:.3f}".format(acc_cond_clean, acc_sample_dist_adv))
        if not self.config.training.use_DDP or \
                (self.config.training.use_DDP and self.config.training.local_rank == 0):
            f = open(self.config.model.attack + '.txt', 'a')
            f.write("The final acc_clean:{:.3f}, acc_adv:{:.3f}".format(acc_cond_clean, acc_sample_dist_adv) + '\n')
