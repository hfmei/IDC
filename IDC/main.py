...
def parse_args_and_config():
    ...
    parser.add_argument('--param_path', type=str, default="", help="The directory to save results with params")
    ...
    parser.add_argument('--dataset_path', type=str, default='./datasets', help='Load Path of dataset')
...
def DDP_run_fn(rank, world_size, config):
    try:
        # logger
        args = config.args
        log_path = os.path.join(args.result_path, config.data.dataset_name, config.model.model_name, args.param_path)
        ...
        print('using device:', config.training.device)
        config.training.local_rank = local_rank
        runner = get_runner(config.runner, config, logger)
