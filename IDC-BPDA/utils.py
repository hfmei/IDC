...
def load_data(args, adv_batch_size):
    if 'imagenet' == args.domain:
        val_dir = args.dataset_path + '/imagenet_lmdb/val'  # using imagenet lmdb data
    ...
    elif 'cifar10' == args.domain:
        data_dir = args.dataset_path
        transform = transforms.Compose([transforms.ToTensor()])
        val_data = data.cifar10_dataset_sub(args, data_dir, transform=transform,
                                            num_sub=args.num_sub, data_seed=args.data_seed)
        n_samples = len(val_data)
        val_loader = DataLoader(val_data, batch_size=n_samples, shuffle=False, pin_memory=True, num_workers=4)
        x_val, y_val = next(iter(val_loader))
    elif 'celebahq' == args.domain:
        data_dir = args.dataset_path + '/celebahq'
    ...
    elif 'cifar100' == args.domain:
        data_dir = args.dataset_path
        transform = transforms.Compose([transforms.ToTensor()])
        val_data = data.cifar100_dataset_sub(args, data_dir, transform=transform,
                                            num_sub=args.num_sub, data_seed=args.data_seed)
        n_samples = len(val_data)
        val_loader = DataLoader(val_data, batch_size=n_samples, shuffle=False, pin_memory=True, num_workers=4)
        x_val, y_val = next(iter(val_loader))
