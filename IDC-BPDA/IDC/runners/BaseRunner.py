...
from IDC.evaluation.FID import calc_FID
from IDC.evaluation.LPIPS import calc_LPIPS
from IDC.runners.base.EMA import EMA
from IDC.runners.utils import make_save_dirs, make_dir, get_dataset_proto, remove_file
...
class ResNet_Model(nn.Module):
    def __init__(self):
        super().__init__()
        # image classifier
        self.resnet = load_model(model_name='Standard', dataset='cifar10', threat_model='Linf')
...
class BaseRunner(ABC):
    ...
    def calculate_epoch(self, train_loader, val_loader, prototypes):
        ...
        # (x_cond_train, x_train, label_train) = train_loader
        # (x_cond_val, x_val, label_val) = val_loader
        ...
        for batch in pbar:
            ...
            for i in range(len(sample)):
                ...
                # y_sample = torch.argmax(classifier(x_sample), dim=-1)

                x_x_cond = x_cond[i: i+1]
                y_sample_dist = torch.argmin(torch.mean((x_sample.repeat(
                    prototypes.size()[0], 1, 1, 1) - prototypes).abs(),dim=(1, 2, 3)))
                y_cond = torch.argmax(classifier(x_x_cond), dim=-1)
                # if y_sample.item() == label[i]:
                #     acc_sample = acc_sample + 1.0
                if y_cond.item() == label[i]:
                    acc_cond = acc_cond + 1.0
                if y_sample_dist.item() == label[i]:
                    acc_sample_dist = acc_sample_dist + 1.0

        num_total = self.config.data.val.batch_size * len_acc
        # acc_sample = acc_sample / num_total
        acc_cond = acc_cond / num_total
        acc_sample_dist = acc_sample_dist / num_total
        self.restore_ema()

        return acc_cond, acc_sample_dist
                    ...
                    for train_batch in pbar:
                        ...
                        if len(self.optimizer) > 1:
                            ...
                            pbar.set_description(
                                (
                                    f'Epoch: [{epoch + 1} / {self.config.training.n_epochs}] '
                                    f'iter: {self.global_step} lr: {lr} loss: {losses[0]:.4f}'
                                )
                            )
                ...
                if (epoch + 1) % self.config.training.validation_interval == 0 or (
                    ...
                    if not self.config.training.use_DDP or \
                        ...
                        with torch.no_grad():
                            ...
                            self.logger.info("Calculating accuracy epoch...")
                            acc_cond, acc_sample_dist = self.calculate_epoch(train_loader, val_loader, val_dataset.prototypes)
                            self.logger.info("Epoch[{:d}], acc_cond:{:.3f}, acc_sample_dist:{:.3f}".format(
                                epoch, acc_cond, acc_sample_dist))
                            torch.cuda.empty_cache()
                            ...
                            if self.config.args.save_top:
                                ...
                                if top_key not in self.topk_checkpoints:
                                    ...
                                    self.logger.info(f"saving top checkpoint: average_loss={average_loss} epoch={epoch + 1}"
                                          f"acc_cond={acc_cond}  acc_sample_dist={acc_sample_dist}")
                                    torch.save(model_states,
    ...
    def test(self):
        ...
        if self.config.args.sample_to_eval:
            ...
            if self.config.training.use_DDP:
                self.sample_to_eval(self.net.module, test_loader, sample_path, test_dataset.prototypes)
            else:
                self.sample_to_eval(self.net, test_loader, sample_path, test_dataset.prototypes)
        else:
